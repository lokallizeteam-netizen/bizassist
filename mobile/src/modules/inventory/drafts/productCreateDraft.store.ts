...
// ✅ Draft passthrough key (Create Item draft stability)
...
export type CategorySelectionReturnParams = {
	[CATEGORY_SELECTED_ID_KEY]?: string; // empty string means "None" (explicit clear)
...
/**
 * Picker inbound params may include selection keys for:
 * - highlighting the current selection
 * - auto-commit after create (selectionSource="created")
 * - draft passthrough for Create Item persistence
 */
...
export function parseCategorySelectionParams(raw: {
	...
	selectedCategoryId: string; // "" allowed (explicit clear)
	selectedCategoryName: string;
	selectionSource?: CategorySelectionSource;
